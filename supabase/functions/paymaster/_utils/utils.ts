import {
  entryPoint06Address as ENTRYPOINT_ADDRESS_V06,
  UserOperation,
} from "npm:viem/account-abstraction";
//These are diff than docs above... mention in Discord
import {
  Address,
  BlockTag,
  Hex,
...
export async function willSponsor({
  ...
  // check chain id
  if (chainId !== optimism.id) return false;
  // check entrypoint
  ...
  try {
    ...
    if (!code) {
      ...
      if (!userOp.initCode) return false; // Check if initCode is undefined
